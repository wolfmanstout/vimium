...
minimizedHintsAreEnabled = ->
  return isEnabledForUrl? and not window.location.toString().match(/https:\/\/mail.google.com/g) and not window.location.toString().match(/https:\/\/critique.corp.google.com/g) and not window.location.toString().match(/https:\/\/docs.google.com/g) and not window.location.toString().match(/https:\/\/colab.research.google.com/g)
...
HintCoordinator =
  ...
  clearMutations: ->
    if @mutationObserver then @mutationObserver.takeRecords()

  refreshHints: ->
    # Only refresh when minimized to avoid disruption.
    if not (@linkHintsMode?.mode == SHOW_MINIMIZED) then return

    # Rate-limit refreshes.
    MIN_INTERVAL = 500
    now = new Date().getTime()
    if @lastRefreshMs and now < (@lastRefreshMs + MIN_INTERVAL)
      if not @refreshTimeoutId
        @refreshTimeoutId = setTimeout((-> HintCoordinator.refreshHints()),
                                       (@lastRefreshMs + MIN_INTERVAL) - now)
      return
    @lastRefreshMs = now
    if @refreshTimeoutId
    ...
    # Perform the refresh.
    console.log("Refreshing minimized hints")
...
class LinkHintsMode
  ...
  constructor: (hintDescriptors, @mode = OPEN_IN_CURRENT_TAB) ->
    ...
    # Clear any mutations generated by the link hints display.
    HintCoordinator.clearMutations()
